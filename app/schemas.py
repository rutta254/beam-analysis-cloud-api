...
class BeamAnalysisRequest(BaseModel):
    ...
    supports: Optional[Supports] = None  # Defaults to simple support at ends if None
    E: Optional[float] = Field(210e6, description="Modulus of Elasticity in kPa (default: 210 GPa for structural steel)", gt=0)
    ...
    @field_validator("varying_loads")
    @classmethod
    def validate_varying_loads(cls, loads: List[VaryingLoad], info) -> List[VaryingLoad]:
        if "length" in info.data:
            L = info.data["length"]
            for load in loads:
                if load.end > L:
                    raise ValueError(f"Varying load end at {load.end}m exceeds beam length of {L}m")
        return loads
